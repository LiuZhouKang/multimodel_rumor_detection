...
REM 进行数据集的处理
E:\Anaconda3\envs\rumor\python.exe -u data_prepare.py --ratio 0.2 --data_from weibo --shuffle_ratio 0.5
if errorlevel 1 goto :error
REM ratio用于处理的数据集占总数据集的比例(默认"0.2")，data_from表示数据集的来源(默认"weibo")
REM shuffle_ratio用于部分打乱匹配的比例(默认"0.0"表示全匹配，若是消融实验可以设为非0值，如0.5)
...
REM 可以在此命令后增加 --disable_match 来不进行全文匹配检测，例如:
REM E:\Anaconda3\envs\rumor\python.exe -u get_mixed_feature.py --data_from weibo --disable_match，否则不写最后一个参数
E:\Anaconda3\envs\rumor\python.exe -u get_mixed_feature.py --data_from weibo --disable_match
if errorlevel 1 goto :error
REM data_from表示数据集的来源(默认"weibo")
REM 注意事项：用于保存的文件夹名称记得修改或确认是否存在

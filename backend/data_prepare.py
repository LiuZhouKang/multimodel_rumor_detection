...
def get_weibo_datasets(ratio=0.2, enhance_images=True):
    ...
    # 保存得到的数据集
    print("------------------数据处理完毕，得到数据集----------------")
    os.makedirs("~/multimodel_rumor_detection/weibo_datasets", exist_ok=True)
    np.save("~/multimodel_rumor_detection/weibo_datasets/train_image_path.npy", train_image_path)
    np.save("~/multimodel_rumor_detection/weibo_datasets/train_text.npy", train_text)
    np.save("~/multimodel_rumor_detection/weibo_datasets/train_label.npy", train_label)
    np.save("~/multimodel_rumor_detection/weibo_datasets/test_image_path.npy", test_image_path)
    np.save("~/multimodel_rumor_detection/weibo_datasets/test_text.npy", test_text)
    np.save("~/multimodel_rumor_detection/weibo_datasets/test_label.npy", test_label)
    print("-----------------------数据集保存完毕---------------------")

def load_weibo_datasets():
    '''加载微博的数据集'''
    train_image_path=np.load("~/multimodel_rumor_detection/weibo_datasets/train_image_path.npy")
    train_text=np.load("~/multimodel_rumor_detection/weibo_datasets/train_text.npy")
    train_label=np.load("~/multimodel_rumor_detection/weibo_datasets/train_label.npy")
    test_image_path=np.load("~/multimodel_rumor_detection/weibo_datasets/test_image_path.npy")
    test_text=np.load("~/multimodel_rumor_detection/weibo_datasets/test_text.npy")
    test_label=np.load("~/multimodel_rumor_detection/weibo_datasets/test_label.npy")
...
def save_datasets(train_image_path, train_text, train_label, test_image_path, test_text, test_label):
    """保存数据集"""
    # 确保保存目录存在
    os.makedirs("~/multimodel_rumor_detection/Twitter_datasets", exist_ok=True)

    print("------------------数据处理完毕，得到数据集----------------")
    np.save("~/multimodel_rumor_detection/Twitter_datasets/train_image_path.npy", train_image_path)
    np.save("~/multimodel_rumor_detection/Twitter_datasets/train_text.npy", train_text)
    np.save("~/multimodel_rumor_detection/Twitter_datasets/train_label.npy", train_label)
    np.save("~/multimodel_rumor_detection/Twitter_datasets/test_image_path.npy", test_image_path)
    np.save("~/multimodel_rumor_detection/Twitter_datasets/test_text.npy", test_text)
    np.save("~/multimodel_rumor_detection/Twitter_datasets/test_label.npy", test_label)
    print("-----------------------数据集保存完毕---------------------")
...
def load_twitter_datasets():
    """加载推特的数据集"""
    train_image_path = np.load("~/multimodel_rumor_detection/Twitter_datasets/train_image_path.npy", allow_pickle=True)
    train_text = np.load("~/multimodel_rumor_detection/Twitter_datasets/train_text.npy", allow_pickle=True)
    train_label = np.load("~/multimodel_rumor_detection/Twitter_datasets/train_label.npy", allow_pickle=True)
    test_image_path = np.load("~/multimodel_rumor_detection/Twitter_datasets/test_image_path.npy", allow_pickle=True)
    test_text = np.load("~/multimodel_rumor_detection/Twitter_datasets/test_text.npy", allow_pickle=True)
    test_label = np.load("~/multimodel_rumor_detection/Twitter_datasets/test_label.npy", allow_pickle=True)
    return train_image_path, train_text, train_label, test_image_path, test_text, test_label
